...
let dateHours: number = dateTime.getHours();
...
const MainTitleDiv: HTMLElement = document.getElementById("mainTitle")!;
...
const BtnElement: HTMLElement = document.querySelector(".btnDiv")!.querySelector("button")!;
const WelcomeParag: HTMLElement = document.createElement("p");
 ...
 */
function toggleWelcomeMessage(): void {
  const BtnDiv: HTMLElement = BtnElement.parentElement!;
  if (BtnDiv.querySelector("p")) {
    BtnDiv.querySelector("p")?.remove();
  } else {
    BtnElement.parentElement?.append(WelcomeParag);
  }
}
...
const StudentsList: Array<Student> = STUDENTS_LIST_DTO.map((s) => new Student(s));
...
const DemoDiv: HTMLElement = document.getElementById("demo")!;
...
const StudentsNameAgeLocationList: Array<string> = StudentsList.map((s) => getNameAgeLocation(s));
 ...
 */
function getNameAgeLocation(s: Student): string {
...
const StudentsNameAgeLocationUlElement: HTMLElement = document.createElement("ul"); // Unordered List
StudentsNameAgeLocationList.forEach((s) => {
  const StudentNameAgeLocationLiElement: HTMLElement = document.createElement("li"); // List Item
...
const StudentsCountriesList: Array<string> = [];
...
const StudentsCountriesParag: HTMLElement = document.createElement("p");
let studentsCountries: string = StudentsCountriesList.join(", ");
...
const StudentsLanguagesList: Array<string> = 
  Array.from(new Set(StudentsList.map(s => s.languages).reduce((acc, val) => acc.concat(val), []))).sort();
...
let mapLanguagesAndStudents = new Map<string, Array<string>>();
StudentsLanguagesList.forEach((l) => {
  const ListSameLanguageStudents: Array<string> = [];
  StudentsList.forEach((s) => {
...
mapLanguagesAndStudents.forEach((students, language) => {
  const LanguageParag: HTMLElement = document.createElement("p");
  // Capitalizzo la prima lettera della stringa
  LanguageParag.innerHTML = language.charAt(0).toUpperCase() + language.slice(1);

  const StudentsSameLanguageUlElement: HTMLElement = document.createElement("ul");

  students.forEach((s) => {
    const StudentLiElement: HTMLElement = document.createElement("li");
...
class LangMap {
  ...
  constructor(lang: string, studs: Array<string>) {
    this.langName = lang;
    this.studsNames = studs;
  }
}

const ListStudentsLanguages: Array<string> = [];
...
const MapLanguagesStuds: Array<LangMap> = [];
let index = 0;
ListStudentsLanguages.forEach((lan) => {
  MapLanguagesStuds.push(new LangMap(lan, new Array<string>()));
  StudentsList.forEach((stu) => {
